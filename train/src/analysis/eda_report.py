...
def summarize(
    *,
    ...
    image_0_sequences = (
        [d for d in image_0.iterdir() if d.is_dir()] if image_0.exists() else []
    )
    image_2_sequences = (
        [d for d in image_2.iterdir() if d.is_dir()] if image_2.exists() else []
    )

    label_sequences = (
        [d for d in labels_root.iterdir() if d.is_dir()] if labels_root.exists() else []
    )
    labels_per_sequence = {d.name: _count_labels(d) for d in label_sequences}
    labels_txt_total = sum(labels_per_sequence.values())

    manual_review_rows = _read_manual_review_rows(
        labels_root / "manual_review_required.csv"
    )
    ...
    for split_name in ("train", "val", "test"):
        ...
        labels = (
            sum(1 for p in lbl_dir.glob("*.txt") if p.is_file())
            if lbl_dir.exists()
            else 0
        )
        stems_match = False
