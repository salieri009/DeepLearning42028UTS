...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Pseudo-label JRDB subset using YOLOv8."
    )
    parser.add_argument(
        "--model", type=str, default="yolov8m.pt", help="YOLO model path or name"
    )
    parser.add_argument(
        "--src-dir",
        type=Path,
        default=_REPO / "data/raw/images",
        help="Source images directory",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=_REPO / "data/processed/labels",
        help="Output labels directory",
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable visual sanity check previews"
    )
    parser.add_argument(
        "--debug-dir",
        type=Path,
        default=_REPO / "data/processed/debug_previews",
        help="Debug previews directory",
    )
    parser.add_argument(
        "--conf-thresh",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.6,
        help="Confidence threshold below which manual review is flagged",
    )
    parser.add_argument(
        "--imgsz",
        type=int,
        default=640,
        help="Inference image size (longer side); larger -> better recall but slower",
    )
    parser.add_argument(
        "--iou",
        type=float,
        default=0.7,
        help="NMS IoU threshold; lower -> fewer overlapping boxes (good for crowds)",
    )
    parser.add_argument(
        "--augment",
        action="store_true",
        help="Enable Test-Time Augmentation (TTA) for higher recall at extra cost",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "JSON checkpoint file path "
            "(default: <out-dir>/pseudo_label_checkpoint.json)"
        ),
    )
    parser.add_argument(
        "--checkpoint-interval",
...
def write_yaml(output_dir: Path, class_map: dict[str, int]) -> Path:
...
def write_checkpoint(
    ...
    *,
    ...
    images_per_second = (
        images_processed / elapsed_seconds if elapsed_seconds > 0 else 0.0
    )
    eta_seconds = (
        (remaining_images / images_per_second) if images_per_second > 0 else None
    )
...
def main() -> None:
    ...
    if args.no_clearml:
        ...
        try:
            task: Any = Task.init(
                project_name="CrowdNav", task_name="pseudo_labeling_v1"
            )
    ...
    # Quarter batch when TTA is on (memory x4) or halve when imgsz > 640.
    if args.augment:
    ...
    print(
        "Using device: "
        f"{device} | Batch Size: {batch_size} | imgsz: {args.imgsz} | "
        f"iou: {args.iou} | augment: {args.augment}"
    )
    ...
    sequences = [
        d for d in src_dir.iterdir() if d.is_dir() and d.name in {"image_0", "image_2"}
    ]
    ...
    for seq_group in sequences:
        for sequence in seq_group.iterdir():
            ...
            image_paths = sorted(
                [img for img in sequence.iterdir() if img.suffix.lower() in valid_exts]
            )
    ...
    for i in range(0, total_run_target, batch_size):
        ...
        for r_idx, r in enumerate(results):
            ...
            if needs_review:
                manual_review_list.append(
                    [str(img_path), lowest_conf if lowest_conf is not None else 0.0]
                )
        ...
        if (
            total_images_processed % checkpoint_interval == 0
            or total_images_processed == total_run_target
        ):
            write_checkpoint(
                checkpoint_path,
    ...
    print(
        f"Total processed: {total_images_processed} images, "
        f"{total_boxes_written} boxes."
    )
    final_elapsed = max(0.001, time.time() - run_started)
    print(
        f"Elapsed: {final_elapsed / 60.0:.2f} min | "
        f"Throughput: {total_images_processed / final_elapsed:.2f} img/s"
    )
