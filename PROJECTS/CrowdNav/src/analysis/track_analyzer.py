...
class TrackAnalyzer:
    ...
    def __init__(self, fps: float = 15.0):
        self.fps = fps
        self.dt = 1.0 / fps
        self.tracks: Dict[int, List[Tuple[float, float, float]]] = {} # {id: [(frame, x, y)]}

    def add_observation(self, track_id: int, frame_idx: int, x: float, y: float):
        if track_id not in self.tracks:
            self.tracks[track_id] = []
        self.tracks[track_id].append((frame_idx, x, y))

    def calculate_velocity(self, track_id: int, window: int = 5) -> Tuple[float, float, float, float]:
        ...
        if df == 0: return 0.0, 0.0, 0.0, 0.0
    ...
    def get_summary(self) -> pd.DataFrame:
        # Logic to return a summary of all tracks
        pass
...
if __name__ == "__main__":
    ...
    print(f"ID 1: Velocity=({vx:.2f}, {vy:.2f}), Speed={speed:.2f}, Heading={heading:.2f} deg")
