...
flowchart TD
    ...
    subgraph PSEUDO["🤖 YOLOv8 Pseudo-Labeling\n(src/data/pseudo_label_yolov8.py)"]
        ...
        C4["Write YOLO .txt labels\n→ data/processed/labels/<seq>/"]
        C5{"conf >= 0.8?"}
        C6["✅ Auto-labeled"]
    ...
    subgraph OUTPUT["📁 Processed Output"]
        ...
        E3["data.yaml (auto-generated)"]
        E4["debug_previews/ (--debug flag)"]
    end
    ...
    subgraph SAGEMAKER["☁️ AWS SageMaker Training"]
...
```
- 좌표는 0.0~1.0 사이로 정규화된 값 (이미지 크기 대비 상대 좌표).
- `class_id=0` = Person.
...
task.connect(vars(args))  # 모든 CLI 파라미터 자동 로깅
```
...
| `data.yaml` | AWS SageMaker 학습 설정 파일 |
...
### Step 7 — AWS SageMaker Training ☁️
...
```bash
# 학습 실행 예시 (deploy/train_skeleton.py 기반)
