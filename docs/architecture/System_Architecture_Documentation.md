...
flowchart LR
    ...
    subgraph CIQualityGate
      ...
      Ruff[ruff check train/src]
      Mypy[mypy --strict train/src]
    end
...
```
  ...
  sequenceDiagram
    ...
    TP->>TP: train / validate
    TP->>Art: export optional onnx/torchscript
  ```
...
- CI runs `ruff` / `mypy --strict` on `train/src` (see `.github/workflows/build-check.yml`).
...
- Add deployment view for Docker compose runtime under `infra/docker/`.
